...
export class RegistrDialogComponent implements AfterViewInit, OnDestroy {
  ...
  constructor(private userService: UserService, private dialogRegistration: MatDialogRef<RegistrDialogComponent>,) {}
  ...
  ngAfterViewInit(): void {
    const eventUserSubscription = this.form.newItemEvent.subscribe(user => {
      ...
      let userLoginSubscription = this.userService.checkLogin(userLogin).subscribe(val => {

              if (val) {
                this.userService.addNewUser(user);
                this.onClose();
              } else {
                this.form.loginMessage = 'Login exist, try another';
              }
